...
EOS
        target.merge_duplicate_zip_code do |row|
          ...
          expect(row.choiki).to eq('choiki1（choiki2）')
        end
      end
      
      it "町域カナについては全く同じ値の場合はマージされないこと" do
        create_temp_file(<<EOS)
x,x,zip,x,x,aa1,x,x,choiki1（,x,x,x,x,x,x
x,x,zip,x,x,aa1,x,x,choiki2）,x,x,x,x,x,x
EOS
        target.merge_duplicate_zip_code do |row|
          expect(row.zip_code).to eq('zip')
          expect(row.choiki_kana).to eq('aa1')
          expect(row.choiki).to eq('choiki1（choiki2）')
        end
      end
